...
// List handlers
const selectListItem = (ev, listName: string) => {
...
document.querySelector('#name-sl').addEventListener('click',  () => {
    document.querySelector('#speakers-list-group li.active')?.classList.remove('active');
});
document.querySelector('#name-sl').addEventListener('input',  () => {
    if ((document.querySelector('#name-sl') as HTMLInputElement).value.trim() === '') {
        document.querySelector('#add-button-sl').setAttribute('disabled', 'true');
    } else {
