...
class Timer {
    constructor(doneCallback, ms = 0, interval, intervalCallback) {
        this._doneCallback = () => {
            doneCallback();
        };
...
const startStopButton = document.getElementById('start-stop-timer-btn');
...
const createTimer = (mins, secs) => {
    const totalMs = (mins * 60 + secs) * 1000;
    return new Timer(doneCallback, totalMs, 1000, intervalCallback);
};
// Start/stop button
...
resetButton.addEventListener('click', () => {
    currMins = agendaMins;
    currSecs = agendaSecs;
    myTimer = new Timer(doneCallback, currSecs * 1000, 1000, intervalCallback);
    timerDisplay.textContent = `${currMins}:${currSecs < 10 ? '0' : ''}${currSecs}`;
    startStopButton.textContent = 'Start';
    resetButton.disabled = true;
